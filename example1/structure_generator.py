...
def compression_cycle(atot, btot, wtot, d_all, cc_settings, fit_settings, uq_settings):

    # Get compression cycle settings.
    ncycles = cc_settings["ncycles"]
    nsteps = cc_settings["nsteps"]
    nchecks = cc_settings["nchecks"]
    ncheck_every = cc_settings["ncheck_every"]
    assert(ncheck_every == int(nsteps/nchecks))
    compress_ratio = cc_settings["compress_ratio"]
    velocity_seed = cc_settings["velocity_seed"]
    fh_log = cc_settings["logfile"]
    ...
    for cycle in range(ncycles):
        ...
        lmp.command(f"fix 2 all deform 1 x final 0.0 {compress_ratio*xlength} units box")
        ...
        xlength = lengths[0]
        lmp.command("unfix 2")
        lmp.command(f"fix 2 all deform 1 x final 0.0 {xlength_start} units box")
        lmp.command("uncompute snap")
        lmp.command("compute snap all snap 4.67637 0.99363 6 0.5 1.0 rmin0 0.0 bzeroflag 1 quadraticflag 0 switchflag 1 bnormflag 0 wselfallflag 0 bikflag 1")
        lmp.command(f"run 0 stop {stopstep}")
        uq = calc_uq_peratom(lmp, fs, d_all)
